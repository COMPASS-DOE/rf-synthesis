...
myboxplot_p <- function(my_factor, var, ylab, my_title){
...
plot_grid(myboxplot_p(predictors, nse, "NSE", "Predictor variables"), 
          myboxplot_p(model, nse, "NSE", "Model package"), 
          myboxplot_p(proportion, nse, "NSE", "Train:test dataset ratio"), 
          myboxplot_p(m_try, nse, "NSE", "Variables per split"), 
          myboxplot_p(ntree, nse, "NSE", "No. of trees"), 
...
ggsave("figures/SI-feature_importance_all_predictors.pdf", width = 8, height = 6)
...
ggplot(var_comparison, aes(as.factor(n_predictors), nse)) + 
  geom_boxplot() + 
  labs(x = "Number of predictors", y = "NSE") + 
  facet_wrap(~data)
ggsave("figures/SI-variable_combo_metrics_combined.pdf", width = 6, height = 4)
